...
export default function Comment() {
    ...
    const fetchComments = async () => {
        ...
        try {
            const res = await fetch("https://tiktok-clone-backend-hb85.onrender.com/users/profile/", {
                headers: {
    ...
    const handleLikeComment = async (commentId) => {
        try {
            await fetch(`https://tiktok-clone-backend-hb85.onrender.com/posts/comments/${commentId}/like_toggle/`, {
                method: "POST",
    ...
    const handleLikeReply = async (replyId) => {
        try {
            await fetch(`https://tiktok-clone-backend-hb85.onrender.com/posts/comments/${replyId}/like_toggle/`, {
                method: "POST",
                headers: { "Authorization": `Bearer ${token}` }
            });
            fetchComments();
                    ...
                    filteredComments.map(comment => (
                        <div key={comment.id} className="single-comment-wrapper">
                            <div className="comment-left-side">
                                <img
                                    src={comment.user?.avatar ? `https://tiktok-clone-backend-hb85.onrender.com${comment.user.avatar}` : "https://via.placeholder.com/40"}
                                        ...
                                        <div className="nested-replies-area">
                                            {comment.reply_comments.map(reply => (
                                                <div key={reply.id} className="single-reply-item">
                                                    <img
                                                        src={reply.user?.avatar ? `https://tiktok-clone-backend-hb85.onrender.com${reply.user.avatar}` : "https://via.placeholder.com/32"}
                            ...
                            {comment.post && (
                                <div className="post-preview-box">
                                    {comment.post.video ? (
                                        <video
                                            src={`https://tiktok-clone-backend-hb85.onrender.com${comment.post.video}`}
                                            className="post-media-preview"
                                            muted
                                        />
                                    ) : comment.post.image ? (
                                        <img
                                            src={`https://tiktok-clone-backend-hb85.onrender.com${comment.post.image}`}
