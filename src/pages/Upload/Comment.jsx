...
export default function Comment() {
    ...
    const handleLikeComment = async (commentId) => {
        try {
            await fetch(`http://localhost:8000/posts/comments/${commentId}/like_toggle/`, {
                method: "POST",
    ...
    const handleLikeReply = async (replyId) => {
        try {
            await fetch(`http://localhost:8000/posts/comments/${replyId}/like_toggle/`, {
                method: "POST",
                headers: { "Authorization": `Bearer ${token}` }
            });
            fetchComments();
                    ...
                    filteredComments.map(comment => (
                        <div key={comment.id} className="single-comment-wrapper">
                            <div className="comment-left-side">
                                <img
                                    src={comment.user?.avatar ? `http://localhost:8000${comment.user.avatar}` : "https://via.placeholder.com/40"}
                                        ...
                                        <div className="nested-replies-area">
                                            {comment.reply_comments.map(reply => (
                                                <div key={reply.id} className="single-reply-item">
                                                    <img
                                                        src={reply.user?.avatar ? `http://localhost:8000${reply.user.avatar}` : "https://via.placeholder.com/32"}
                            ...
                            {comment.post && (
                                <div className="post-preview-box">
                                    {comment.post.video ? (
                                        <video
                                            src={`http://localhost:8000${comment.post.video}`}
                                            className="post-media-preview"
                                            muted
                                        />
                                    ) : comment.post.image ? (
                                        <img
                                            src={`http://localhost:8000${comment.post.image}`}
